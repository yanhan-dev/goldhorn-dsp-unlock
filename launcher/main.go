...
// QML source text patches (equal length) - for decompressed text AND compressed rewrite
var textPatches = []struct {
	old []byte
	new []byte
}{
...
// Pre-computed patched compressed block (generated by Python, padded to original size)
//go:embed patched_block.bin
var patchedBlock []byte

// Zlib signature for the verification QML block
var zlibSig = []byte{0x78, 0xda, 0xec, 0xbd}
const origCompressedSize = 99124

// Phase 1: Find compressed QML block by signature, overwrite with pre-computed patch
func patchCompressedBlocks(hProc syscall.Handle) int {
	var addr uintptr
	...
	for {
		...
		if mbi.State == MEM_COMMIT &&
			...
			if bytesRead > 0 {
				...
				offset := 0
				for {
					idx := bytes.Index(data[offset:], zlibSig)
					if idx == -1 { break }
					absOff := offset + idx
					blockAddr := mbi.BaseAddress + uintptr(absOff)

					// Verify via Qt resource header: 8 bytes before = [totalSize BE][decompSize BE]
					if absOff >= 8 {
						hdr := data[absOff-8 : absOff]
						totalSize := int(hdr[0])<<24 | int(hdr[1])<<16 | int(hdr[2])<<8 | int(hdr[3])
						compSize := totalSize - 4
						if compSize == origCompressedSize {
							// Match! Overwrite with pre-computed patched block
							var oldProt uint32
							procVirtualProtectEx.Call(uintptr(hProc), blockAddr, uintptr(origCompressedSize),
								PAGE_EXECUTE_READWRITE, uintptr(unsafe.Pointer(&oldProt)))
							var written uintptr
							procWriteProcessMemory.Call(uintptr(hProc), blockAddr,
								uintptr(unsafe.Pointer(&patchedBlock[0])), uintptr(origCompressedSize),
								uintptr(unsafe.Pointer(&written)))
							procVirtualProtectEx.Call(uintptr(hProc), blockAddr, uintptr(origCompressedSize),
								uintptr(oldProt), uintptr(unsafe.Pointer(&oldProt)))
							if written > 0 {
								appendLog(fmt.Sprintf("  Patched compressed block @0x%X (%d bytes)", blockAddr, written))
								return 45 // known patch count
							}
						}
					}
					offset = absOff + 4
...
// Diagnostic: show what patterns exist in process memory
func diagScan(hProc syscall.Handle) {
	var addr uintptr
	var mbi MEMORY_BASIC_INFORMATION
	mbiSize := unsafe.Sizeof(mbi)
	...
	regions := 0
	totalBytes := uint64(0)
	foundASCII := 0    // "isNeedVerifyDevice" as ASCII
	foundExpr := 0     // "Ams.isNeedVerifyDevice()" as ASCII
	foundZlib := 0     // 78 DA zlib headers
	foundZlibSig := 0  // our specific 78 DA EC BD signature
	...
	for {
		...
		if mbi.State == MEM_COMMIT &&
			...
			if bytesRead > 0 {
				...
				off := 0
				for {
					idx := bytes.Index(data[off:], []byte("isNeedVerifyDevice"))
					if idx == -1 { break }
					foundASCII++
					off += idx + 18
				}

				// Count full expression matches
				off = 0
				for {
					idx := bytes.Index(data[off:], []byte("Ams.isNeedVerifyDevice()"))
					if idx == -1 { break }
					foundExpr++
					off += idx + 24
				}
				...
				off = 0
				for {
					idx := bytes.Index(data[off:], []byte{0x78, 0xda})
					if idx == -1 { break }
					foundZlib++
					if off+idx+4 <= len(data) && data[off+idx+2] == 0xec && data[off+idx+3] == 0xbd {
						foundZlibSig++
						appendLog(fmt.Sprintf("  zlib sig at 0x%X", mbi.BaseAddress+uintptr(off+idx)))
					}
					off += idx + 2
				}
			}
	...
	appendLog(fmt.Sprintf("Scanned %d regions, %.1f MB", regions, float64(totalBytes)/1048576))
	appendLog(fmt.Sprintf("Found: ASCII 'isNeedVerifyDevice'=%d, full expr=%d", foundASCII, foundExpr))
	appendLog(fmt.Sprintf("Found: zlib 78DA=%d, our sig 78DAECBD=%d", foundZlib, foundZlibSig))
}
...
func doLaunchAndPatch() {
	...
	go func() {
		...
		appendLog("Racing to patch before Qt loads QML...")
		procPostMessage.Call(uintptr(hwndMain), WM_APP_SETSTATUS, 3, 0)

		// Race: scan every 100ms to patch compressed QML BEFORE Qt reads it
		// UPX decompresses in ~200ms, Qt loads QML at ~500-2000ms
		totalPatched := 0
		for attempt := 0; attempt < 100; attempt++ { // up to 10 seconds
			var exitCode uint32
			procGetExitCodeProcess.Call(hMem, uintptr(unsafe.Pointer(&exitCode)))
			if exitCode != STILL_ACTIVE { break }

			count := patchCompressedBlocks(syscall.Handle(hMem))
			if count > 0 {
				totalPatched += count
				appendLog(fmt.Sprintf("Patched compressed QML (%d items, attempt #%d)", totalPatched, attempt+1))
		...
		if totalPatched == 0 {
			appendLog("WARNING: compressed block not found in time")
		}

		// Phase 2: Continuous text-level scanning as backup
		appendLog("Monitoring...")
		for {
			var exitCode uint32
			procGetExitCodeProcess.Call(hMem, uintptr(unsafe.Pointer(&exitCode)))
			if exitCode != STILL_ACTIVE { break }
			...
			if count > 0 {
				totalPatched += count
				appendLog(fmt.Sprintf("Phase 2: patched %d more (total: %d)", count, totalPatched))
				procPostMessage.Call(uintptr(hwndMain), WM_APP_SETSTATUS, 2, uintptr(totalPatched))
			}
